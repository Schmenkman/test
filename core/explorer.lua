...
local function move_to_target()
    ...
    if not target then
        console.print("No valid target found.")
        return
    end

    current_path = a_star(get_player_position(), target)
    if current_path then
        path_index = 1
        console.print("Path found to target.")
    else
        console.print("No path found to target.")
    end
end
...
function explorer:set_custom_target(pos)
    target_position = pos
    console.print("Custom target set to: " .. tostring(pos))
end
